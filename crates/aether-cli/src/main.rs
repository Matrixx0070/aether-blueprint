...
\n\
  /help for key bindings  ·  /model to switch AI  ·  /cost for usage",
                                ...
                                "/init" => {
                                    ...
                                    let aether_md = cwd.join("AETHER.md");
                                    let claude_md = cwd.join("CLAUDE.md");
                                    let target = if claude_md.exists() {
                                        claude_md.clone()
                                    } else {
                                        aether_md.clone()
                                    };
                                    if target.exists() {
                                        let fname = target.file_name().and_then(|n| n.to_str()).unwrap_or("AETHER.md");
                                        ui.chat_lines.push(ChatLine::SystemNote(
                                            format!("{fname} already exists in this directory.\n  Edit it directly, or delete it and run /init again.")
                                        ));
                                    } else {
...
\n\
> Auto-generated by `aether /init`. Edit freely — Aether reads this on every startup.\n\
\n\
## Project\n\
\n\
**Name:** {project_name}\n\
...
## Tech Stack\n\
\n\
<!-- e.g. Rust 2024 · Tokio · Ratatui · Serde -->\n\
\n\
## Key Files\n\
\n\
...
## Conventions\n\
\n\
<!-- Code style, test commands, PR workflow, etc. -->\n\
\n\
## Aether Preferences\n\
\n\
<!-- Instructions for Aether:\n\
- Use /alias to define project-specific shortcuts\n\
- Use /pin-cmd to set a persistent context prefix\n\
-->\n\
");
        ...
        // Priority: (1) history prefix match, (2) @-path file completion.
            ...
            && !ui.input_buffer.contains('\n')
        {
            ...
            // 1. History prefix completion
            let hist = ui.input_history.iter().rev().find_map(|h| {
            ...
            if hist.is_some() {
                ...
                // 2. @-path file completion: complete the path after the last @ token
                buf.rfind('@').and_then(|at_pos| {
                    let partial = &buf[at_pos + 1..];
                    // Only complete single-token paths (no spaces after @)
                    if partial.contains(char::is_whitespace) || partial.is_empty() {
                        return None;
                    }
                    let cwd = std::env::current_dir().unwrap_or_default();
                    let (dir, stem) = if partial.contains('/') {
                        let p = std::path::Path::new(partial);
                        let parent_dir = p.parent()
                            .map(|pa| if pa.as_os_str().is_empty() { cwd.clone() } else { cwd.join(pa) })
                            .unwrap_or(cwd);
                        let fname = p.file_name().and_then(|n| n.to_str()).unwrap_or("");
                        (parent_dir, fname.to_string())
                    } else {
                    ...
                    std::fs::read_dir(&dir).ok().and_then(|rd| {
                        let mut matches: Vec<String> = rd
                            .filter_map(|e| e.ok())
                            .filter_map(|e| e.file_name().into_string().ok())
                            .filter(|n| n.starts_with(&stem) && n.len() > stem.len())
                            .collect();
                        matches.sort();
                        matches.into_iter().next().map(|first| first[stem.len()..].to_string())
                    })
                })
            }
        } else {
