...
pub enum UiCommand {
    ...
    QueryEnvValueMaxLen,
    /// Show max length of any session var value.
    QuerySessionVarValueMaxLen,
    /// Show max length of any alias expansion value.
    ...
    /// Show count of pending (incomplete) progress items.
    QueryProgressItemsPendingCount,
    /// Show the first pending (incomplete) progress item.
    ...
    /// Show the highest turn index with a bookmark.
    QueryBookmarkTurnMax,
    /// Show the lowest turn index with a history annotation.
    ...
    /// Show the shortest turn label string.
    QueryTurnLabelMinLen,
    /// Show the shortest bookmark label string.
    ...
    /// Show the shortest previous output in tool output history.
    QueryToolOutputHistPrevMinLen,
    /// Show the shortest current output in tool output history.
    ...
    /// Show the maximum output tokens in a single turn.
    QueryTurnOutTokensMax,
    /// Show the minimum input tokens in a single turn.
    ...
    /// Show the shortest tool name key in tool output history.
    QueryToolOutputHistKeyMinLen,
    /// Show the longest block type key in plan block counts.
    ...
    /// Show the count of pending reminders from Source::ProjectHook.
    QueryPendingReminderProjectHookCount,
    /// Show the count of pending reminders from Source::UserHook.
    ...
    /// Show the average previous output length in tool_output_history.
    QueryToolOutputHistPrevAvgLen,
    /// Show the max absolute length delta between curr and prev in tool_output_history.
    QueryToolOutputHistDeltaMax,
    /// Show the average key length in tool_output_history.
    QueryToolOutputHistKeyAvgLen,
    /// Show the total chars across all keys in tool_output_history.
    QueryToolOutputHistKeyTotalChars,
    /// Show the min absolute length delta between curr and prev in tool_output_history.
    ...
    /// Show whether overlay section D3 is enabled.
    QueryOverlaySectionD3,
    /// Show whether overlay section D4 is enabled.
    ...
    /// Average description length across all verifier gate rules.
    QueryVerifierGateRuleDescAvgLen,
    /// Average compiled_regexes count per verifier gate rule.
    ...
    /// Total pattern count across all PhraseMatch detector rules.
    QueryVerifierGatePhrasePatternTotal,
    /// Total pattern count across all Regex detector rules.
    ...
    /// Average chars per User turn in the session history.
    QueryHistoryUserAvgChars,
    /// Average chars per Assistant text turn in the session history.
    ...
    /// Total chars across all tool_use names in the session history.
    QueryHistoryToolUseNameTotalChars,
    /// Count of distinct tool names used in the session history.
    ...
    /// Number of block keys in plan.block_counts.
    QueryPlanBlockCount,
    /// Sum of ok-counts from plan.tool_call_stats.
    ...
    /// Max length of any tool-use ID in conversation history.
    QueryHistoryToolUseIdMaxLen,
    /// Min length of any tool-use ID in conversation history.
    ...
    /// Minimum word count in any single session note.
    QuerySessionNoteMinWords,
    /// Total word count across all history annotations.
    ...
    /// Approximate sentence count in the plan text (split on '. ', '! ', '? ').
    QueryPlanTextSentenceCount,
    /// Average chars per word in the plan text.
    ...
    /// Total word count across all error_playbook hints.
    QueryErrorPlaybookHintWordTotal,
    /// Average words per error_playbook hint.
    ...
    QueryTurnLabelGapAvg,
    /// Maximum turn-index gap between any two consecutive turn labels.
    ...
    /// Average absolute cost difference between consecutive turn_cost_log entries.
    QueryTurnCostLogCostDiffAvg,
    /// Maximum absolute cost difference between consecutive turn_cost_log entries.
    ...
    /// Average absolute output-token difference between consecutive turn_cost_log entries.
    QueryTurnCostLogOutDiffAvg,
    /// Maximum absolute output-token difference between consecutive turn_cost_log entries.
    ...
    QueryLabelsPerTurn,
    /// Ratio of error_playbook entries to history turn count.
    QueryPlaybookPerTurn,
    /// Ratio of session_vars count to history turn count.
    QueryVarsPerTurn,
    ...
    /// Ratio of auto_tag_rules count to history turn count.
    QueryAutoTagsPerTurn,
    /// Ratio of saved_snapshots count to history turn count.
    ...
    /// Minimum word count in any single user-role history turn.
    QueryHistoryUserMinWords,
    /// Minimum word count in any single assistant-role history turn.
    ...
    QueryP99InTokens,
    /// Median (p50) output tokens per cost-log entry.
    QueryP50OutTokens,
    /// 90th-percentile output tokens per cost-log entry.
    QueryP90OutTokens,
