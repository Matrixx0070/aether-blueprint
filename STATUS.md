...
| Y-audit | DONE | 9d474b8 | FIXED | 33/33 Y-tests pass after B1+B2+B3+H1+H2 audit fixes; live smoke re-run → Y7 LIVE-VERIFIED OK | pre-tag audit: cert-pin + XSW + RelayState-bail + local-name + Recipient/InResponseTo |
...
| CC4 | DONE | b3e334b | n/a | live smoke 5-step chain: configure-saml v1 → sso-saml.json captures sha256 metadata_fingerprint (64 hex); refresh-saml on unchanged v1 → "no drift ... skipping layout rewrite" + pem mtime unchanged; flip to v2 → drift detected + rewrote 2 certs + pem mtime advanced; third refresh on still-v2 → "no drift" (post-rewrite fingerprint stable). 6 unit tests cover order-insensitive cert sorting + NUL separators + cert/non-cert field changes + fingerprint persistence | SAML metadata drift detection (closes BB6 weakest-point) |
| CC5 | DONE | 6e73b97 | n/a | live smoke 4-step chain: login → sso.access_token.expires_at sidecar at 0600 (RFC 3339 UTC, ~3600s delta); whoami outside window (lead=60s, 3600s remaining) → events=['userinfo'] no proactive refresh; whoami inside window (lead=3600s) → events=['token','userinfo'] proactive refresh hits /token BEFORE /userinfo + stderr "proactive refresh (CC5)"; --no-refresh suppresses both proactive and reactive. 4 unit tests cover already-expired + within-lead + outside-lead + env-knob clamp | OIDC proactive refresh (closes BB5 weakest-point) |
| CC6 | DONE | 963a0f5 | n/a | live smoke 5-step chain: Ed25519 PKCS#8 SP key written at 0600; sso login with AETHER_SAML_SP_PRIVATE_KEY_PEM → signed AuthnRequest carries eddsa-ed25519 SignatureMethod URI and NOT RSA-SHA256; full spec-path verify with cryptography (Reference URI match + DigestValue == sha256(c14n(unsigned)) + 64-byte Ed25519 SignatureValue verifies under SP pubkey); Y3-Y7 IdP→SP leg unchanged → sso.token at 0600. 3 unit tests (Ed25519 PEM load + EdDSA URI dispatch + end-to-end round-trip with ed25519_dalek::Verifier); 3 BB4 tests migrated to SpSigningKey::Rsa wrapper | EdDSA AuthnRequest signing (closes BB4 weakest-point) |
| CC7 | DONE | 9424798 | n/a | v0.33.0 ship: workspace bump + 15 internal dep pins + ROADMAP + STATUS + Plan DD draft + tag + push; run 28286470290 4-platform autobuild green (~8m10s longest leg); cosign verify-blob → "Verified OK"; ./aether --version → aether 0.33.0 from published artifact | Plan CC wrap-up + v0.33.0 ship — every AA + BB weakest-point closed |
| DD4 | DONE | d9b95b5 | n/a | live smoke 5-step chain: Ed25519 IdP cert (RFC 8410) → Ed25519-signed SAMLResponse → Y5 algorithm gate accepts eddsa-ed25519 → per-key dispatch picks Ed25519 verifying key (skips wrong-type RSA keys per risk register) → Verifier::verify on raw c14n SignedInfo bytes succeeds → Y3-Y7 composition through sso.token at 0600. 5 unit tests: happy path + EdDSA-sig-against-RSA-cert + RSA-sig-against-Ed25519-cert (both fail closed with "tried 0, skipped 1") + algorithm-gate URL rejection + SPKI byte round-trip. 3 Y5/AA5 tests migrated to IdpVerifyingKey::Rsa wrapper | Y5 EdDSA assertion verifier (closes CC6 weakest-point) |
| DD5 | DONE | 4a25cac | n/a | live smoke 6-step chain: far-future validUntil → configure-saml OK + sso-saml.json carries valid_until; drift+far-future → rewrite no WARN; near-expiry (12h, default 24h window) → no-drift + WARN cites remaining seconds; PAST validUntil → refresh-saml bails nonzero + idp-certs/ untouched (mtime+contents); absent validUntil → advisory line; AETHER_SAML_METADATA_STALENESS_WARN_SECS=3600 + 30m-out → WARN cites 3600s. 6 unit tests cover helpers + env knob + parse + apply-bails-on-expired + persists round-trip. 2 CC4 tests migrated to add valid_until: None | SAML metadata validUntil staleness check (closes CC4 follow-up gap) |
| DD6 | DONE | 9b29306 | n/a | live smoke 5-step chain: in-sync clocks (skew=0s) → sidecar at 0600 + no WARN; server-in-past (offset -120s) → skew ≈ +120s + WARN cites threshold 60s; server-in-future via manual sso refresh (offset +90s) → skew ≈ -90s + WARN; custom AETHER_OIDC_CLOCK_SKEW_WARN_SECS=10 → WARN cites threshold 10s; logout removes sso.clock_skew_secs alongside the other 3. 3 unit tests cover RFC 7231 IMF-fixdate parse + signed-seconds math + env-knob clamp. Caught mid-development: Python http.server auto-adds Date header in send_response; overrode date_time_string() on the smoke handler so the framework's emission carries the configured offset | OIDC system-clock-skew detection (closes CC5 weakest-point) |
| DD7 | DONE | c64feed | n/a | v0.34.0 ship: workspace bump + 15 internal dep pins + ROADMAP + STATUS + Plan EE draft + tag + push; run 28287542883 4-platform autobuild green (~8m); cosign verify-blob SHA256SUMS → "Verified OK"; sha256sum --check → OK; ./aether --version → aether 0.34.0 from published linux-x86_64 artifact | Plan DD wrap-up + v0.34.0 ship — AA→BB→CC→DD chain complete across all 3 lanes |
| EE4 | DONE | 5e89e8f | n/a | tests/check-status-no-placeholders.sh bash gate (set -euo pipefail; grep -Fn so parens are literal; exit 0/1/2; AETHER_SKIP_STATUS_PLACEHOLDER_CHECK=1 escape per risk register §EE4). Wired into release.yml as new prerelease-checks job; build job needs: prerelease-checks so tag push with placeholder never starts 4-platform build. 4-case live verify: clean → exit 0, injected placeholder → exit 1 with grep line + 3-line refusal, escape hatch → exit 0 with skip message, missing file → exit 2. Caught real historical Y-audit row placeholder (SHA never backfilled, line 168) → backfilled to 9d474b8 in same commit | Pre-tag CI placeholder check (closes Y7→DD7 chronic STATUS-row pattern) |
| EE5 | DONE | 034e1f7 | n/a | live smoke 5-step chain: cacheDuration=PT2H persisted as cache_duration_secs=7200 + watch banner cites "every 7200s (source: cacheDuration"; no attribute → null + banner cites "every 3600s (source: default"; env=120 → "every 120s (source: env"; garbage env + PT2H hint → "every 7200s (source: cacheDuration" (the fix). 6 unit tests: xsd:duration happy paths (P1D/PT1H/PT30M/PT1H30M/P1Y6M/P1DT12H) + month-vs-minute T disambiguation + garbage rejection (empty/-P1D/P1X/PT./P1H out-of-position) + parse-extract present+absent + picker precedence matrix with source-string assertions + apply persist round-trip. Caught mid-development by S5: source-string logic in watch loop diverged from picker (said "env" even when garbage env value did NOT influence result) — refactored saml_metadata_refresh_interval_secs to return (u64, &'static str) so source + interval are one decision. 1 BB6 test migrated; 5 CC4+DD5 struct literals migrated | SAML metadata cacheDuration support (closes DD5 weakest-point) |
| EE6 | DONE | d4ed0ef | n/a | live smoke 5-step chain: Ed448 IdP cert (RFC 8410 OID 1.3.101.113 + 57B pubkey assertion) → 444B cert PEM written; Ed448-signed SAMLResponse (sig=114B asserted) with SignatureMethod URI eddsa-ed448; algorithm gate accepts; per-key dispatch picks Ed448 verifying key (skips wrong-type Ed25519/RSA keys per DD4 risk register); ed448_goldilocks::VerifyingKey::verify_raw on raw c14n SignedInfo bytes succeeds; Y3-Y7 composition through sso.token at 0600 with saml.v1. prefix. 5 unit tests + 1 helper: happy path + Ed448-sig-against-Ed25519-trust + Ed448-sig-against-RSA-trust (both fail closed with "tried 0, skipped 1") + algorithm-gate URL rejection citing all 3 accepted URIs + SPKI 57-byte round-trip pinning PUBLIC_KEY_LENGTH constant. Crate: ed448-goldilocks v0.14.0-pre.15 (RustCrypto org) with signing+getrandom features. Trust assumption (audit gap per risk register §EE6): Ed448 far less battle-tested than Ed25519 — documented inline at verify call-site | Ed448 SAML verify path (closes DD4 weakest-point) |
| EE7 | DONE | 7c0cbee | n/a | v0.35.0 ship: workspace bump + internal dep pins + ROADMAP + STATUS + Plan FF draft + tag pushed 2026-06-27. Publish was BLOCKED on GitHub Actions billing at tag time (run 28289226581 passed prerelease-checks + 4 platform builds, failed only at publish-github-release); billing cleared and `gh workflow run release.yml --ref v0.35.0 -F tag=v0.35.0` (run 28553407420) published the release 2026-07-01: cosign verify-blob SHA256SUMS → "Verified OK"; sha256sum --check → OK; ./aether --version → aether 0.35.0 from published linux-x86_64 artifact | Plan EE wrap-up + v0.35.0 ship (backfilled post-publish per the billing-block note) |
